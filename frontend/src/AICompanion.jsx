...
function Face({ mood = "neutral", speaking = false, listening = false }) {
  const eye = "rounded-full w-4 h-4 bg-slate-100"; const mouthBase = "h-1 rounded-full bg-slate-100";
...
export default function AICompanion() {
  ...
  async function handleSend(text) {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ message: content }) }); const data = await resp.json();
  ...
  function startListening() {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SR) { alert("Speech Recognition not supported in this browser. Try Chrome or Edge desktop."); return }
